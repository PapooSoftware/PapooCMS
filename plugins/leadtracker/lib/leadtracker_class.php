...
{
	/**
	 * leadtracker_class constructor.
	 */
	function __construct()
	{
		global $content, $db, $checked, $diverse, $user, $cms, $weiter, $mail_it, $db_abs;
	...
	public function post_papoo()
	{
		...
		if (isset($this->checked->download_form) && $this->checked->download_form=="ok") {
	...
	function make_details()
	{
		...
		if ($tab == 'adwords') {
			$this->content->template['adwords'] = $this->get_adwords($this->checked->cookie_id);
		}
		...
		if (strpos($accept, 'fragment=#subcontent') !== FALSE) {
			...
			elseif ($tab == 'adwords') {
				$template = str_replace('/leadtracker_backend_details.html', '/leadtracker_backend_details_sub_adwords.html', $template);
			}
			else {
	...
	/**
	 * @param null $search_mail
	 * @param null $search_visits
	 * @param null $search_forms
	 * @param bool $skip_empty_mail
	 * @param int $use_weiter
	 * @param null $search_last_visit
	 ...
	 * @param $cookie_id
	 * @param int $use_weiter
	 * @param bool $include_three_values
	 * @param null $since
	 * @param null $until
	 * @return array|void
	 */
	 ...
	 * @return array|void
	 */
	function get_adwords($cookie_id, $use_weiter=20)
	{
		$where = array();
		$where[] = "(`leadtracker_track_campaignid` <> 0";
		$where[] = "(`leadtracker_track_adgroupid` <> 0";
		$where[] = "(`leadtracker_track_keyword` <> ''";
		if(!empty($cookie_id)) {
			$where = "WHERE " . implode(" AND `leadtracker_track_cookie` = '" .
					$this->db->escape($cookie_id) . "') OR ", $where) . " AND `leadtracker_track_cookie` = '" . $this->db->escape($cookie_id) . "')";
		}
		else {
			$where = "WHERE " . implode(" AND `leadtracker_track_cookie` <> '') OR ", $where) . " AND `leadtracker_track_cookie` <> '')";
		}
		if ($use_weiter) {
			$sql = sprintf("SELECT COUNT(*) FROM `%s` %s",
				$this->cms->tbname['plugin_leadtracker_tracking'],
		...
		$sql = sprintf("SELECT
                            `leadtracker_track_cookie` AS `yummy`,
                            `leadtracker_track_timestamp` AS `timestamp`,
                            `leadtracker_track_url` AS `referrer`,
                            `leadtracker_track_campaignid` AS `campaignid`,
                            `leadtracker_track_adgroupid` AS `adgroupid`,
                            `leadtracker_track_keyword` AS `keyword`
                        FROM `%s` %s ORDER BY `leadtracker_track_timestamp` DESC %s",
			$this->cms->tbname['plugin_leadtracker_tracking'],
			$where,
			($use_weiter)?$this->weiter->sqllimit:''
		);
		...
		foreach($result as &$row) {
			if(!$row['referrer']) {
				...
				preg_match_all('/\?(.*)$/U', $row['referrer'], $matches);
				$matches = explode('&', $matches[1][0]);
				foreach($matches as $row2) {
					$hilf = explode('=', $row2);
					if(strtolower($hilf[0]) == 'campaignid' || strtolower($hilf[0]) == 'adgroupid' || strtolower($hilf[0]) == 'keyword') {
						if (stristr($row['referrer'], $row2)) {
							$row['referrer'] = str_replace('?' . $row2 . '&', '?', $row['referrer']);
							$row['referrer'] = str_replace('&' . $row2, '', $row['referrer']);
							$row['referrer'] = str_replace('?' . $row2, '', $row['referrer']);
						}
					}
	...
	function make_formdetails()
	{
		...
		IfNotSetNull($tab); // FIXME: was war $tab mal?
		if ($tab == 'pages') {
			$this->content->template['pages'] = $this->get_pages($this->checked->cookie_id);
		}
		if ($tab == 'downloads') {
	...
	{
		if(isset($this->checked->send_mails) && $this->checked->send_mails) {
			$this->get_and_send_due_fum();
		}
		$this->content->template['cron_guard'] = 'cron-'.substr(md5(gethostname().PAPOO_ABS_PFAD.'$cronjob'), 0, 6);
		$this->content->template['template'] = 'leadtracker/templates/leadtracker_follow_up.html';
		if(isset($this->checked->delete) && $this->checked->delete) {
		...
		if(isset($this->checked->post_del) && $this->checked->post_del) {
		...
		$tmpl_string = "";
		$countsql = "";
		if(isset($this->checked->search_mail) && $this->checked->search_mail) {
		...
		if(isset($this->cms->tbname['papoo_form_manager_lead_content'])) {
			$sql = sprintf("SELECT
			...
			$sql = sprintf("SELECT
						...
						FROM
							...
							%s AS t4
						WHERE
							t1 . lookup_lead_fum_fum_id = t4 . leadtracker_fum_id
							GROUP BY t1 . lookup_lead_fum_id",
				$this->cms->tbname['plugin_leadtracker_lookup_lead_fum'],
				$this->cms->tbname['plugin_leadtracker_follow_up_mails']
			);
