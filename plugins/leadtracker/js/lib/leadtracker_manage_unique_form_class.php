...
class leadtracker_manage_unique_form_class
{
	function __construct()
	{
        ...
        if ( defined("admin") )
        {
            ...
            if ( $template != "login.utf8.html")
            {
                ...
                {
                    if($this->checked->check_autorefill)
                    {
    ...
    public function post_papoo_leadtracker()
    {
        ...
        //debug::print_d($vorhanden);
        //return true;

        //debug::print_d($vorhanden);
        //debug::Print_d($_SESSION['sonderfall_form_leadtracker']);
        ...
        if (empty($vorhanden)) {
            ...
            if($autofill[0]['leadtracker_autofill']) {
                ...
                //debug::print_d($_SESSION['first_form_data']);
                // echo "-";
                //exit("HU");
            ...
            if (!empty($this->checked->reload_form)) {
             ...
             //debug::print_d($_SESSION['formdat']);
    }
	...
	private function erzeuge_korekte_zuweisung($lead_data=array())
    {
        ...
        if (!empty($_SESSION['first_form_data'])  && is_numeric($this->checked->rldif) && $this->checked->fertig!=1)
        {
            ...
            {
                //debug::Print_d($v['form_manager_content_lead_feld_name']);
    ...
    private function make_formular_verknuepfung()
    {
        ...
        if (is_numeric($this->checked->set_new_form_del))
        {
            //Verjnuepfung lösen
        ...
        //Neues übernehmen
        if (is_numeric($this->checked->set_new_form))
        {
        ...
        //debug::Print_d($this->content->template['leadtracker_nicht_verknuepfte_forms']);
    }
