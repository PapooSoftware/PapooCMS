...
class class_create_site
{
	...
	function create_page($id)
	{
		...
		$sql = sprintf( "SELECT * FROM %s
										WHERE kalender_id='%d'", $this->cms->tbname['plugin_landing_page_sites'],
			$this->db->escape( $id ) );
		$result = $this->db->get_results( $sql, ARRAY_A );

		//Die FE Startseite im gew�hlten Design rausholen
		$this->get_data_frontend( $result['0']['kalender_design'] );
		...
		$this->diverse->write_to_file( "/plugins/landing_pages/sites/" . $this->
			verzeichnis . "/index.html", $this->index_html );
		@chmod(PAPOO_ABS_PFAD . "/plugins/landing_pages/sites/" . $this->verzeichnis . "/index.html", 0777);
	}
	...
	function get_data_frontend( $style = 0 )
	{
		...
		$daten1 = explode( '<!-- ### start_of_content -->', $daten );
		$daten2 = explode( '<!-- ### end_of_content -->', $daten1['1'] );
	...
	function create_verzeichniss( $result = array() )
	{
		...
		$file = str_replace( ".", "_", $result['0']['kalender_domain'] );
		$this->verzeichnis = $file = preg_replace( "/[^a-zA-Z0-9_]_/", "", $file );

		//Das gesamte vorhanden Verzeichnis l�schen
		global $diverse;
		$diverse->rec_rmdir( PAPOO_ABS_PFAD . "/plugins/landing_pages/sites/" . $file );

		//Wenn noch nicht vorhanden anlegen
		if ( !is_dir( PAPOO_ABS_PFAD . "/plugins/landing_pages/sites/" . $file ) ) {
			mkdir( PAPOO_ABS_PFAD . "/plugins/landing_pages/sites/" . $file );
			@chmod( PAPOO_ABS_PFAD . "/plugins/landing_pages/sites/" . $file, 0777 );

			mkdir( PAPOO_ABS_PFAD . "/plugins/landing_pages/sites/" . $file . "/css" );
			@chmod( PAPOO_ABS_PFAD . "/plugins/landing_pages/sites/" . $file . "/css", 0777 );

			mkdir( PAPOO_ABS_PFAD . "/plugins/landing_pages/sites/" . $file . "/js" );
			@chmod( PAPOO_ABS_PFAD . "/plugins/landing_pages/sites/" . $file . "/js", 0777 );

			mkdir( PAPOO_ABS_PFAD . "/plugins/landing_pages/sites/" . $file . "/images" );
			@chmod( PAPOO_ABS_PFAD . "/plugins/landing_pages/sites/" . $file . "/images",
				0777 );

			mkdir( PAPOO_ABS_PFAD . "/plugins/landing_pages/sites/" . $file . "/images/thumbs" );
			@chmod( PAPOO_ABS_PFAD . "/plugins/landing_pages/sites/" . $file . "/images/thumbs",
				0777 );
		}

		//Dann die CSS Verzeichnisse kopieren in Verzeichnis /css
		$sql = sprintf("SELECT * FROM %s WHERE style_id='%d' ", DB_PRAEFIX .
		...
		$dest = PAPOO_ABS_PFAD . "/plugins/landing_pages/sites/" . $file . "/css/";
		...
		@copy( PAPOO_ABS_PFAD . "/loader.gif" , PAPOO_ABS_PFAD .
			"/plugins/landing_pages/sites/" . $file . "/loader.gif"  );
		@chmod( PAPOO_ABS_PFAD . "/plugins/landing_pages/sites/" . $file . "/loader.gif"
			, 0777 );
		...
		if ( is_array( $img ) ) {
			foreach ( $img['0'] as $key => $value_img ) {
				...
				@copy( PAPOO_ABS_PFAD . "" . $value, PAPOO_ABS_PFAD .
					"/plugins/landing_pages/sites/" . $file . "/images/" . $pop );
				@chmod( PAPOO_ABS_PFAD . "/plugins/landing_pages/sites/" . $file . "/images/" .
					$pop, 0777 );
			}
		}
				...
				if ( is_array( $js2 ) ) {
					foreach ( $js2['2'] as $key2 => $value2 ) {
						...
						@copy( PAPOO_ABS_PFAD . "" . $value2, PAPOO_ABS_PFAD .
							"/plugins/landing_pages/sites/" . $file . "/js/" . $pop );
						if ( file_exists( PAPOO_ABS_PFAD . "/plugins/landing_pages/sites/" . $file .
							"/js/" . $pop ) ) {
							@chmod( PAPOO_ABS_PFAD . "/plugins/landing_pages/sites/" . $file . "/js/" . $pop,
								0777 );
						}
					}
				}
		...
		if ( is_array( $css ) ) {
			foreach ( $css['2'] as $key => $value ) {
				if ( stristr( $value, "css" ) ) {
					...
					$pop = array_pop( $v_ar );
					@copy( PAPOO_ABS_PFAD . "/" . $value, PAPOO_ABS_PFAD .
						"/plugins/landing_pages/sites/" . $file . "/css/" . $pop );
					if ( file_exists( PAPOO_ABS_PFAD . "/plugins/landing_pages/sites/" . $file .
						"/css/" . $pop ) ) {
						@chmod( PAPOO_ABS_PFAD . "/plugins/landing_pages/sites/" . $file . "/css/" . $pop,
							0777 );
					}
				}
			}
		...
		if ( is_array( $css2 ) ) {
			foreach ( $css2['0'] as $key => $value ) {
				if ( stristr( $value, "templates_c" ) ) {
					...
					@copy( PAPOO_ABS_PFAD . "/" . $value, PAPOO_ABS_PFAD .
						"/plugins/landing_pages/sites/" . $file . "/css/" . array_pop( $v_ar ) );

					@chmod( PAPOO_ABS_PFAD . "/plugins/landing_pages/sites/" . $file . "/css/" .
						array_pop( $v_ar ), 0777 );
				}
			}
		}
...
$class_create_site = new class_create_site();
