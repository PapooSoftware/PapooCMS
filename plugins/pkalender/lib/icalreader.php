...
class ICal
{
	...
	/**
     * Return DateTime object from ical date time format
     *
     * @param {string} $icalDate A Date in the format YYYYMMDD[T]HHMMSS[Z] or
     *                           YYYYMMDD[T]HHMMSS
     * @param {string} $fromtimezone Source Timezone if not UTC. Defaults to system timezone
     * @return {DateTime}
     */
    public function iCalDateToDateTime($icalDate, $fromtimezone=NULL)
    {
		$isutc = strpos($icalDate, 'Z') !== FALSE;
        $icalDate = str_replace('Z', '', $icalDate);
		
		if ($isutc) $tzstr = 'UTC';
		else $tzstr = ($fromtimezone === NULL)?date_default_timezone_get():$fromtimezone;
		
		$tz = new DateTimeZone($tzstr);
		return new DateTime($icalDate, $tz);
    }
     ...
     *                           YYYYMMDD[T]HHMMSS
     * @param {string} $fromtimezone Source Timezone if not UTC. Defaults to system timezone
	 * @param {string} $totimezone Destination Timezone. Defaults to system timezone
     * @return {array} Associative Array: time, time_stamp
     */
    public function iCalDateToUnixTimestamp($icalDate, $fromtimezone=NULL, $totimezone=NULL)
    {
		$dt = $this->iCalDateToDateTime($icalDate, $fromtimezone);
		
		if ($totimezone === NULL) $totimezone = date_default_timezone_get();
		$dt->setTimezone(new DateTimeZone($totimezone));
		
		$time = (strlen($icalDate) > 10) ? $dt->format('H:i') : NULL;
        return array( 'time' => $time, 'time_stamp' => $dt->format('U'));
    }
