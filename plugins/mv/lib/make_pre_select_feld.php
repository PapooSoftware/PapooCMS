	...
	if ((is_array($cdaten)))
	{
		foreach($cdaten as $daten)
		{
				...
				&& $daten->content != "")
			{
				...
				//Hier speziell die Pre Select Trenner nur anzeigen
				$selectwert = $daten->content;
				$daten_ar = explode("+++", $daten->content);

				if (!empty($daten_ar['1'])) $selectwert = $daten_ar['1'];
				$cfeld .= '>' . $selectwert . '</option>';
			}
		}
	}
}
