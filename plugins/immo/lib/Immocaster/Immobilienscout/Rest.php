...
class Immocaster_Immobilienscout_Rest extends Immocaster_Immobilienscout
{
    ...
    {
		parent::__construct($sKey,$sSecret,$sAuth);
    }
	...
	private function _getExpose_images($aArgs)
	{
		$aRequired = array('user');
		$oToken = null;
		$sSecret = null;
		list($oToken, $sSecret) = $this->getApplicationTokenAndSecret();
		
		// $http_url='http://rest.immobilienscout24.de/restapi/api/offer/v1.0/user/601626/realestate/69913084/attachment';
		$req = $this->doRequest('offer/v1.0/user/'.$aArgs['user']."/realestate/".$aArgs['realestate']."/attachment",$aArgs,$aRequired,__FUNCTION__,$oToken);
		$req->unset_parameter('exposeid');
		return parent::getContent($req,$sSecret);
	}
	 ...
	 */
	public function getApplicationTokenAndSecret() {
		$oToken = NULL;
		$sSecret = NULL;
