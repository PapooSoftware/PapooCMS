...
class content_template_class
{
	...
	function do_admin()
	{
		...
		if (defined("admin"))
		{
			IfNotSetNull($this->checked->layout);
		...
		IfNotSetNull($this->content->template['insertok']);
		IfNotSetNull($this->content->template['delok']);
		IfNotSetNull($this->content->template['template_start']);
		IfNotSetNull($this->content->template['template_sys_start']);
		IfNotSetNull($this->content->template['contentnew']);
		IfNotSetNull($this->content->template['template_news_start']);
		IfNotSetNull($this->content->template['contentnew_layout_step2']);
		IfNotSetNull($this->content->template['contentnew_layout_step3']);
		IfNotSetNull($this->content->template['contentnew_layout_step4']);
		IfNotSetNull($this->content->template['contentnew_layout_step5a']);
		IfNotSetNull($this->content->template['contentnew_layout_step5b']);
		IfNotSetNull($this->content->template['contentnew_layout_step1_fehler']);
		IfNotSetNull($this->content->template['contentnew_layout']);
		IfNotSetNull($this->content->template['content_ct']);
	}
	...
	function make_content_template()
	{
		...
		IfNotSetNull($this->checked->update_content);
		IfNotSetNull($this->checked->makenew_content);
		...
		if ($this->checked->update_content=="ok") {
			...
			if ($this->checked->makenew_content=="ok") {
				...
				if (!empty($this->checked->save_ctemplate)) {
					//Daten speichern
					$sql = sprintf("INSERT INTO %s SET ctempl_lang_id='%d', 
									ctempl_name='%s', ctempl_content='%s'",
	...
	{
		$sql=sprintf(" 	SELECT style_pfad FROM %s
							  	WHERE style_id 	='%d'",
	...
	function make_sys_template()
	{
		...
		else if (!empty($this->checked->style_id)) {
			$this->make_file_liste();
		}
	}
		...
		if (!empty($this->checked->layoutname)) {
			...
			if (!$this->check_exist()) {
				...
				if ($this->check_dirs($this->checked->layoutname)==true) {
					...
					$this->content->template['contentnew_layout_step2']="ok"; $this->content->template['contentnew_layout_step2_fehler']="ok";
				}
			}
	...
	{
		//FIXME: $cc war nie gesetzt was hatte man hier vor?
		IfNotSetNull($cc);
