...
class Model
{
  ...
  public function to_csv(array $options=array())
  {
	...
	/**
	 * Funktion die ein array nimmt welches z.B. Model::all() uebergeben wird und ein zweites
	 * array das mit den 'conditions' key des ersten array gemerged werden soll und gibt
	 * das erste Array mit den gemergeten Conditions zurueck.
	 * 
	 * @param array $params
	 * @param array $conditions_to_merge
	 * @return array Fertig gemergetes array
	 *
	 * <code>
	 * $params = ['conditions' => ['id' => 1]];
	 * $conditions_to_merge = ['active' => true];
	 *
	 * $params = Model::merge_conditions($params, $conditions_to_merge);
	 * </code>
	 *
	 * @author Andreas <ag@papoo.de>
	 */
	public static function merge_conditions($params, $conditions_to_merge)
	{
		if(!is_array($conditions_to_merge) or empty($conditions_to_merge)) {
			return $params;
		}

		if(isset($params['conditions'])) 
		{
			if(isset($params['conditions'][0])) 
			{

				if(!isset($conditions_to_merge[0])) 
				{
					$conditions_copy = $conditions_to_merge;
					$conditions_to_merge = [0 => ''];

					$conditions_to_merge[0] = implode(" AND ", array_map(function($key) {
						return "$key = ?";
					}, array_keys($conditions_copy)));

					foreach($conditions_copy as $condition_col => $condition_val)
					{
						$conditions_to_merge[] = $condition_val;
					}
				}

				$params['conditions'][0] = '('.$params['conditions'][0].') AND ('.$conditions_to_merge[0].')';

				for($i = 1; $i < count($conditions_to_merge); $i++)
				{
					if(!isset($conditions_to_merge[$i])) {
						break;
					}

					$params['conditions'][] = $conditions_to_merge[$i];
				}
			}
			else {
				$params['conditions'] = array_merge($params['conditions'], $conditions_to_merge);
			}
		}
		else {
			$params['conditions'] = $conditions_to_merge;
		}
