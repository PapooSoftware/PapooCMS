...
if (strstr( $_SERVER['PHP_SELF'],'ez_sql_mysqli.php')) die('You are not allowed to see this page directly');
...
class ezSQL_mysqli extends ezSQLcore
{
    ...
    {
        //Hier noch spezieller 1und1 check - die haben eine Extrawurst
        if (stristr($dbhost,"mysql5.sock"))
        {
            $dbhost = "localhost";
            $this->mysqlsocket = "/tmp/mysql5.sock";
            $dbport = 3316;
        }
        else
        {
    ...
    function connect($dbuser='', $dbpassword='', $dbhost='localhost', $dbport=false,$mysqlsocket="")
    {
        ...
        {
            $this->dbh = new mysqli($dbhost,$dbuser,$dbpassword, '', $dbport,$this->mysqlsocket);
    ...
    function query($query)
    {
        ...
        // Anpassung vorgenommen am 16.02.2016 <cz>
        if (($this->num_queries + 1) % 1000 == 0)
        {
            //$this->disconnect();
		...
		// @edit 2018-01-25 <cz> Es wurde lediglich nach insert,update,etc am Anfang der Query gesucht, statt Rueckgabewert von mysqli::query zu ueberpruefen
		if ($this->result === true)
        {
            $is_insert = true;
        ...
        // Query was a select
		// @edit 2018-02-22 <cz> Pruefe explizit auf die Klasse, da bei einer leeren $query auch hier reingelaufen wuerde, sofern wie vorher nur else
		...
		// @edit 2018-02-22 <cz> Wenn kein gueltiger Rueckgabewert bzw false, dann raus
		else {
			// sehr wahrscheinlich wird hier immer false returned
			return $this->result;
		}
